...
void Filter::BiquadFilter::recalculateCoefficients()
{
    ...
    */
    ...
    double omega = 2 * PI_DOUBLE * (m_fc/m_fs);
    double sinOmega = std::sin(omega);
    double cosOmega = std::cos(omega);
    double alphaQ = sinOmega / (2 * m_q);
    double alphaBW = sinOmega * std::sinh((log(2) / 2.0) * m_q * (omega / sinOmega));
    double alphaS = (sinOmega / 2.0) * std::sqrt((A + 1 / A) * (1 / m_shelfSlope - 1) + 2);
